...
const sourcesStream = project.sources()
  // splits inline JS & CSS out into individual .js & .css files
...
You can add splitters to any part of your build stream. We recommend using them to optimize both your `sources()` and `dependencies()` streams as in the example above, but you can also add one after merging the two together or even after bundling.


#### Splitting Javascript for CSP Compliance

If you'd like to enable Content-Security-Policy (CSP) rules like 'unsafe-inline', you can configure your HTML splitter to split out inlined JavaScript without rejoining. But some inline styles (like those used by web components) cannot remain split, so some care is required.

There are two ways to accomplish this. The first is to only split out JavaScript, leaving your inline styles untouched in the build stream:

```js
// split inline JS out from HTML
.pipe(htmlSplitter.split(['js'))
// build continues without rejoining
```

The other is to split out both inline JavaScript and CSS, but then to only rejoin the CSS:

```js
// split bpoth inline CSS & JS out from HTML
.pipe(htmlSplitter.split())
// rejoin only the CSS split from HTML, leave split JS files untouched
.pipe(htmlSplitter.rejoin(['css']))
// build continues...
```
